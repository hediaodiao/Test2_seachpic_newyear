    ...
    while retry_count < max_retries:
        try:
            if USE_DOCKER_MILVUS:
                ...
                connections.connect(
                    alias="default",
                    host="standalone",  # 使用docker-compose中的服务名
                    port="19530"
                )
                logging.info("Milvus连接成功")
                return True
            elif USE_MILVUS_LITE:
                # Milvus Lite是本地文件，不需要网络连接
                logging.info("使用Milvus Lite，跳过网络连接检查")
                return True
            else:
                # 本地存储，检查目录是否存在
                if not os.path.exists("./vector_db_storage"):
                    os.makedirs("./vector_db_storage")
                logging.info("本地存储目录检查成功")
                return True
        except Exception as e:
            retry_count += 1
            logging.warning(f"等待依赖服务... ({retry_count}/{max_retries})")
            time.sleep(2)
    ...
    try:
        ...
        if USE_DOCKER_MILVUS or USE_MILVUS_LITE:
            if vector_managers:
                ...
                if test_model in vector_managers:
                    try:
                        # 尝试获取集合统计信息
                        stats = vector_managers[test_model].get_collection_stats()
                        milvus_healthy = True
                    except Exception:
                        pass
        else:
            # 对于本地存储，检查存储目录是否存在
            milvus_healthy = os.path.exists("./vector_db_storage")
        ...
        return JSONResponse(
            ...
            content={
                ...
                "models_loaded": len(extractors) > 0
            }
        )
    except Exception as e:
        return JSONResponse(
